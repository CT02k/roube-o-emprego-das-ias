...
const canRespond = (detail: PromptDetail | null, sessionId: string) => {
  ...
  return (
    !detail.claimInfo.expired &&
    detail.claimInfo.claimedBySessionId === sessionId
  );
};
...
export default function HomePage() {
  ...
  const setImageDraftDataUrl = useUIStore(
    (state) => state.setImageDraftDataUrl,
  );
  ...
  const [shareAvailabilityLoading, setShareAvailabilityLoading] =
    useState(true);
  ...
  const { list, selectedDetail, requesterThread, refresh, isLoading, error } =
    usePromptsData(sessionId, mode, adminUnlocked, adminToken);
  ...
  const { clearSelectedPrompt, selectPrompt, releaseCurrentPrompt } =
    useWorkerClaim({
      mode,
      sessionId,
      selectedPromptId,
  ...
  const onOpenShare = async () => {
    ...
    try {
      ...
      setShareAvailable(false);
      setShareError(
        err instanceof Error ? err.message : "Falha ao gerar card de share.",
      );
    } finally {
  ...
  const reopenByAdmin = async () => {
    ...
    try {
      ...
      setLocalError(
        err instanceof Error ? err.message : "Falha ao reabrir prompt.",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const submitNewPrompt = async (text: string) => {
    ...
    try {
      ...
      setLocalError(
        err instanceof Error ? err.message : "Falha ao enviar prompt.",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const canSendRequester =
    mode === "requester" && textDraft.trim().length > 0 && !isSubmitting;
  const canSendWorker =
    mode === "worker" &&
    draftReady &&
                    ...
                    <div className="p-4">
                      <div className="flex flex-col gap-3 rounded-sm border border-border bg-background p-3">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <Button
                            disabled={
                              textDraft.trim().length === 0 || isSubmitting
                            }
      ...
      <Dialog onOpenChange={setAdminDialogOpen} open={adminDialogOpen}>
        <DialogContent className="max-w-md border-border bg-card">
          <DialogHeader>
            ...
            <DialogDescription>
              Pressione Ctrl + &apos; para abrir este atalho e informe a chave
              de administrador.
            </DialogDescription>
          </DialogHeader>
          <Input
